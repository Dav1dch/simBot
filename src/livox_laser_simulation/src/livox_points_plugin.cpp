...
void LivoxPointsPlugin::OnNewLaserScans() {
  if (rayShape) {
    ...
    for (auto &pair : points_pair) {
      ...
      // angle_min) / angle_incre); if (verticle_index < 0 || horizon_index < 0)
      // {
      //    continue;
      // }
      ...
      ray.Euler(ignition::math::Vector3d(0.0, rotate_info.zenith,
                                         rotate_info.azimuth));
      //                auto axis = rotate * ray * ignition::math::Vector3d(1.0,
      //                0.0, 0.0); auto point = range * axis +
      //                world_pose.Pos();//转换成世界坐标系
