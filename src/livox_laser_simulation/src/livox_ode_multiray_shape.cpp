...
void LivoxOdeMultiRayShape::UpdateRays() {
  ODEPhysicsPtr ode =
      boost::dynamic_pointer_cast<ODEPhysics>(this->GetWorld()->Physics());
  ...
  // especially when spawning models with sensors
  {
    ...
    dSpaceCollide2((dGeomID)(this->superSpaceId), (dGeomID)(ode->GetSpaceId()),
                   this, &UpdateCallback);
  }
}

//////////////////////////////////////////////////
void LivoxOdeMultiRayShape::UpdateCallback(void *_data, dGeomID _o1,
                                           dGeomID _o2) {
  ...
  if (dGeomIsSpace(_o1) || dGeomIsSpace(_o2)) {
    ...
    if (rayCollision && hitCollision) {
      ...
      if (n > 0) {
        RayShapePtr shape =
            boost::static_pointer_cast<RayShape>(rayCollision->GetShape());
...
void LivoxOdeMultiRayShape::AddRay(const ignition::math::Vector3d &_start,
  ...
  ODECollisionPtr odeCollision(
      new ODECollision(this->collisionParent->GetLink()));
...
void LivoxOdeMultiRayShape::Init() {
  ...
  //
  //            // since we're rotating a unit x vector, a pitch rotation will
  //            now be
